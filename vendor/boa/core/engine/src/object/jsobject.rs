...
use super::{
    IsHtmlDdaObject, JsPrototype, NativeObject, Object, ObjectData, PrivateName, PropertyMap,
    internal_methods::{
...
impl JsObject {
    ...
    /// Creates an object that carries Annex B `[[IsHTMLDDA]]` semantics.
    #[inline]
    #[must_use]
    pub fn with_is_html_dda_proto(intrinsics: &Intrinsics) -> Self {
        Self::from_proto_and_data(
            intrinsics.constructors().object().prototype(),
            IsHtmlDdaObject,
        )
    ...
    /// Checks if this object carries Annex B `[[IsHTMLDDA]]` semantics.
    #[inline]
    #[must_use]
    #[track_caller]
    pub fn is_html_dda(&self) -> bool {
        self.is::<IsHtmlDdaObject>()
    }

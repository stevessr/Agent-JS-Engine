...
use icu_calendar::{
    types::MonthCode as IcuMonthCode, AnyCalendarKind, Calendar as IcuCalendar,
};
...
impl MonthCode {
    ...
    pub(crate) fn try_from_fields(
        ...
        match fields {
            ...
            CalendarFields {
                ...
                month_code.validate(calendar)?;
                are_month_and_month_code_resolvable(calendar, era_year, *month, month_code)?;
                Ok(*month_code)
            }
            _ => Err(TemporalError::r#type()
    ...
    pub(crate) fn without_leap(self) -> Self {
        if !self.is_leap_month() {
            return self;
        }

        Self(match self.to_month_integer() {
            1 => MONTH_ONE,
            2 => MONTH_TWO,
            3 => MONTH_THREE,
            4 => MONTH_FOUR,
...
#[inline]
fn are_month_and_month_code_resolvable(
    calendar: &Calendar,
    era_year: Option<&EraYear>,
    month: u8,
    month_code: &MonthCode,
    ...
    let calendar_date = calendar
        .0
        .from_codes(
            era_year.era.as_ref().map(|era| era.0.as_str()),
            era_year.year,
    ...
    if calendar.0.month(&calendar_date).ordinal != month {
        return Err(
            TemporalError::range().with_message("month and monthCode could not be resolved.")
        );
    }

...
const EXACT_DECIMAL_DIGITS: usize = 80;

fn exact_rational_to_f64(numerator: i128, denominator: i128) -> TemporalResult<FiniteF64> {
    debug_assert_ne!(denominator, 0);

    if numerator == 0 {
        return Ok(FiniteF64::default());
    }
    ...
    let integer = numerator_abs / denominator_abs;
    let mut remainder = numerator_abs % denominator_abs;

    if remainder == 0 {
        return FiniteF64::try_from(integer).map(|value| if numerator < 0 { value.negate() } else { value });
    }

    let mut digits = vec![0u8; EXACT_DECIMAL_DIGITS];
    for digit in &mut digits {
        remainder *= 10;
        *digit = (remainder / denominator_abs) as u8;
        remainder %= denominator_abs;
    }

    let decimal: String = digits.into_iter().map(|d| char::from(b'0' + d)).collect();
    let parsed = format!("{sign}{integer}.{decimal}")
        .parse::<f64>()
        .map_err(|_| TemporalError::range())?;
    FiniteF64::try_from(parsed)
}
...
impl TimeDuration {
    ...
    pub(crate) fn total(&self, unit: Unit) -> TemporalResult<FiniteF64> {
        ...
        exact_rational_to_f64(time_duration, unit_nanoseconds.get() as i128)
    }
...
impl InternalDurationRecord {
    ...
    fn nudge_calendar_unit(
        ...
        let midpoint = (r1 + r2) as f64 / 2.0;
        let total_f64 = total.as_inner();
        let expanded = match options.rounding_mode {
            RoundingMode::Expand => total_f64 != r1 as f64,
            ...
            RoundingMode::HalfEven => {
                if sign == Sign::Negative {
                    total_f64 < midpoint || (total_f64 == midpoint && r2 % 2 == 0)
                } else {
                    total_f64 > midpoint || (total_f64 == midpoint && r2 % 2 == 0)
                }
            }
    ...
    fn nudge_to_zoned_time(
        ...
        if self.sign() == Sign::Zero {
            let normalized = InternalDurationRecord::new(DateDuration::default(), TimeDuration::default())?;
            return Ok(NudgeRecord {
                normalized,
                nudge_epoch_ns: start.ns.0,
                total: None,
                expanded: false,
            });
        }

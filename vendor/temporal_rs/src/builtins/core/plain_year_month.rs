...
impl PlainYearMonth {
    ...
    pub(crate) fn add_duration(
        ...
        // For ISO calendars, year/month arithmetic can legally pass through
        // intermediate dates that are outside the PlainDate representable range
        // (for example, subtracting one month from +275760-09). Implement that
        // path directly in year/month space so we don't spuriously reject those
        // cases while still enforcing PlainYearMonth's own representable limits.
        ...
        {
            IsoDate::new_with_overflow(self.iso.year, self.iso.month, 1, Overflow::Constrain)?;

            let year = i64::from(self.iso.year) + duration.years();
            let month = i64::from(self.iso.month) + duration.months();
            let balanced_year = year + (month - 1).div_euclid(12);
            let balanced_month = (month - 1).rem_euclid(12) + 1;

            if balanced_year < i64::from(i32::MIN) || balanced_year > i64::from(i32::MAX) {
                return Err(TemporalError::range().with_message("Exceeded valid range."));
            }

            let balanced_year = balanced_year as i32;
            let balanced_month = balanced_month as u8;

            if !year_month_within_limits(balanced_year, balanced_month) {
                return Err(TemporalError::range().with_message("Exceeded valid range."));
            }

            return Self::new_with_overflow(
                balanced_year,
                balanced_month,
                Some(self.iso.day),
                self.calendar.clone(),
        ...
        if calendar.is_iso() {
            let base = IsoDate::new_with_overflow(date.year, date.month, 1, Overflow::Constrain)?;
            let ym_duration = DateDuration::new_unchecked(duration_to_add.years, duration_to_add.months, 0, 0);
            let moved = base.add_date_duration(&ym_duration, Overflow::Constrain)?;
            if !year_month_within_limits(moved.year, moved.month) {
                return Err(TemporalError::range().with_message("Exceeded valid range."));
            }
            let reference_day = self.iso.day;
            return Self::new_with_overflow(
                moved.year,
                moved.month,
                Some(reference_day),
    ...
    pub(crate) fn diff(
        ...
        // year-month space. This avoids probing out-of-range intermediate dates
        // inside `diff_iso_date()` for edge representable months like
        // `+275760-09` while still honoring the requirement that the first day of
        // each month be within the valid ISO date range.
        let mut duration = if self.calendar().is_iso() {
            let total_months = i64::from(other_iso.year - this_iso.year) * 12
                + i64::from(other_iso.month)
                - i64::from(this_iso.month);

            let date = if resolved.largest_unit == Unit::Year {
                DateDuration::new_unchecked(total_months / 12, total_months % 12, 0, 0)
            } else {
                DateDuration::new_unchecked(0, total_months, 0, 0)
            };

            InternalDurationRecord::from_date_duration(date)?
        } else {
            let result = self
                .calendar()
            ...
            InternalDurationRecord::from_date_duration(result)?
        };

...
fn validate_fractional_seconds_precision(source: &[u8]) -> TemporalResult<()> {
    let mut i = 0;
    while i < source.len() {
        if source[i] == b'.' {
            let mut j = i + 1;
            ...
            if j > i + 10 {
                return Err(TemporalError::range()
                    .with_message("no more than 9 decimal places are allowed"));
            }
            i = j;
            continue;
        }
...
fn parse_ixdtf(source: &[u8], variant: ParseVariant) -> TemporalResult<IxdtfParseRecord<'_, Utf8>> {
    validate_fractional_seconds_precision(source)?;

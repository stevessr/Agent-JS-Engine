...
fn resolved_backref_group<'a, Position: PositionType>(
    groups: &'a mut [GroupData<Position>],
    candidates: &[u32],
) -> Option<&'a mut GroupData<Position>> {
    debug_assert!(!candidates.is_empty(), "Backreference should reference at least one group");

    let mut matched = None;
    let mut fallback = None;
    for &candidate in candidates {
        let group = groups.mat(candidate as usize);
        if group.as_range().is_some() {
            matched = Some(candidate as usize);
        } else if fallback.is_none() {
            fallback = Some(candidate as usize);
        }
    }

    matched.or(fallback).map(|index| groups.mat(index))
}

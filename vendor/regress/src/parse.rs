    ...
    I: Iterator<Item = u32> + Clone,
{
    ...
    fn parse_capture_groups(&mut self) -> Result<(), Error> {
        let orig_input = self.input.clone();
        let mut alternative_named_groups = HashMap::new();
        let mut disjunction_named_groups_stack = Vec::new();
        let mut group_alternative_depths = Vec::new();
            ...
            match self.next().map(to_char_sat) {
                ...
                Some('(') => {
                    ...
                    if self.try_consume('?') {
                        ...
                        {
                            is_capturing = false;
                        } else if let Some(name) = self.try_consume_named_capture_group_name() {
                            named_capture = Some(name);
                        } else {
                            is_capturing = false;
                        }
                    }
                    ...
                    if is_capturing {
                        if let Some(name) = named_capture {
                            if alternative_named_groups.contains_key(&name) {
                                return error("Duplicate capture group name");
                            }
                            alternative_named_groups.insert(name.clone(), self.group_count_max);
                            self.named_group_indices
                                .entry(name)
                                .or_default()
                                .push(self.group_count_max);
                        }
                        group_alternative_depths.push(disjunction_named_groups_stack.len());
                ...
                Some('|') => {
                    let previous_alternative = core::mem::take(&mut alternative_named_groups);
                    disjunction_named_groups_stack.push(previous_alternative);
                }
                Some(')') => {
                    let group_depth = group_alternative_depths.pop().unwrap_or(0);
                    while disjunction_named_groups_stack.len() > group_depth {
                        let mut previous_alternative = disjunction_named_groups_stack.pop().unwrap();
                        previous_alternative.extend(alternative_named_groups.drain());
                        alternative_named_groups = previous_alternative;
                    }
                }
                Some(_) => continue,
                None => break,

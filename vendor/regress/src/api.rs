...
impl<'m> Iterator for NamedGroups<'m> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while self.next_group_idx < end {
            ...
            if self.mat.group_names[..idx].iter().any(|existing| existing.as_ref() == name) {
                continue;
            }

            return Some((name, self.mat.named_group(name)));
        }
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let mut size = 0;
        for name in &self.mat.group_names[self.next_group_idx..] {

...
- 支持 `import-defer` 的兼容子集：`import.defer(...)` 动态调用、`import defer * as ns from ...` 语法降级，以及对应的 test262 dynamic syntax / abrupt-reject / static syntax / syntax-error 组
...
- `import-defer` 子集样本：`68 / 68` 执行通过，另有 `155` 个依赖真实 deferred semantics 的 case 继续跳过

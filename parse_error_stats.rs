...
fn main() {
    ...
    while let Some(path) = stack.pop() {
        if path.is_dir() {
            ...
            match ai_agent::parser::Parser::new(lexer) {
                        ...
                        Err(e) => {
                            ...
                            if err_str.starts_with("Unexpected token: expected ") {
                                if let Some(pos) = err_str.find(", found None") {
                                    ...
                                    let basic_type = token_str.split('(').next().unwrap_or(token_str).to_string();
                                    err_str = err_str[..pos].to_string() + &format!(", found Some({})", basic_type);
                                }
                            }
                ...
                Err(e) => {
                    *general_failures.entry(format!("Lexer error: {:?}", e)).or_insert(0) += 1;
                }
            }
        }
    }
    ...
    println!("Parse Pass Rate: {:.2}% ({}/{})", passed as f64 / total as f64 * 100.0, passed, total);
